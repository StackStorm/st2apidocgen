...
class ExternalSourceMapPromise {
  ...
  constructor(name, buffer = []) {
    ...
    console.log(`  ${chalk.dim('<<<')}${time()} Bundling ${chalk.green(name)}`);
    ...
    this._promise = new Promise((resolve, reject) => {
      ...
      .then((res) => {
        ...
        console.log(`  ${chalk.dim`>>>`}${time()} Done bundling ${chalk.green(name)} [${chalk.dim(hash.digest('hex'))}]`);
        return res;
      })
    ;
  ...
  resolve() {
    ...
    _.forEach(this.buffer, ({ relPath, map, chunk: file }) => {
      ...
      if (match) {
        const embedMap = JSON.parse(new Buffer.from(match, 'base64').toString());
...
function postcssify(file, opts) {
  ...
  if (extensions.indexOf(path.extname(file)) === -1) {
    return stream.PassThrough(); // eslint-disable-line new-cap
  }
