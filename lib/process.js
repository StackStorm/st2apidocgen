...
class ExternalSourceMapPromise {
  constructor(name) {
    ...
    console.log(`  ${chalk.gray('<<< Bundling')} ${name}`);
    ...
    this._promise = new Promise((resolve, reject) => {
      ...
      .then((res) => {
        ...
        console.log(`  ${chalk.gray('>>> Done bundling')} ${name} ${chalk.gray('[' + hash.digest('hex') + ']')}`);
        return res;
      })
      ;
        ...
        require('postcss-modules')({
          ...
          getJSON: (filename, styles) => {
            ...
            Object.entries(styles).forEach(([name, value]) => {
              ...
              const [block, element] = rest.split('__');

              const composite = [block];

              if (element) {
                composite.push(element);
              }
...
module.exports = (files) => {
  ...
  b.on('update', bundle);
  b.on('error', err => console.error(`Unhandled error occured during bundling: ${err}`));
